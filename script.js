...
// SAFE INITIALIZATION METHOD: Guarantees supabase exists when forms use it
...
runSafe(() => {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      // Securely check for admin status via profile data instead of hardcoded email
...
async function loadDashboardData() {
  ...
  // Use getUser() for security as it verifies the JWT with the server
...
async function loadAdminData() {
  ...
  if (adminBody) {
    adminBody.innerHTML = allTransactions.map(tx => `
      <tr>
        <td>${tx.user_id}</td>
        <td><span class="badge badge--${tx.type}">${tx.type.toUpperCase()}</span></td>
