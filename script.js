...
runSafe(() => {
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", async (e) => {
      ...
      alert("Account created successfully! Check your email for a validation link, or proceeding to your dashboard layout.");
...
async function loadDashboardData() {
  ...
  try {
    ...
    // Grab profile data directly from database rows using matching User Session context token
    ...
    // Grab transaction logs securely filtered via active context user UID
...
async function loadAdminData() {
  ...
  const { data: allTransactions, error } = await supabase.from('transactions').select('*');
  if (error) return;
